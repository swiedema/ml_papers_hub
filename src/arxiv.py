...
class Arxiv:
    ...
    def get_papers(self, queries: list[str], verbose: bool = False) -> list[arxiv.Result]:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            future_to_query = {executor.submit(fetch_paper, query): query for query in queries}
    ...
    def download_papers(self, dirpath: str, verbose: bool = False, logger=None):
        ...
        def download_single_paper(paper):
            try:
                ...
                if logger is not None:
                    logger.error(f"Error downloading paper arxiv_id={paper.entry_id} title={paper.title}: {e}")
                else:
                    print(f"Error downloading paper arxiv_id={paper.entry_id} title={paper.title}: {e}")
                return paper

        # Use ThreadPoolExecutor for parallel downloads
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit all papers to the executor
            future_to_paper = {executor.submit(download_single_paper, paper): paper for paper in self.papers}
        ...
        paper_paths = [os.path.join(dirpath, f) for f in os.listdir(dirpath) if f.endswith('.pdf')]
