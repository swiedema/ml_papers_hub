...
def classify_image_with_gpt4v(image_path):
    with open(image_path, "rb") as img_file:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an AI model that classifies images from research papers as figures or tables and extracts captions if available."},
                {"role": "user", "content": "What type of content is in this image? If it's a figure or table, extract any visible captions."},
...
def extract_images_from_pdf(pdf_path, output_folder="extracted_images"):
    ...
    for page_num in range(len(doc)):
        for img_index, img in enumerate(doc[page_num].get_images(full=True)):
            ...
            image_filename = os.path.join(output_folder, f"page_{page_num+1}_img_{img_index+1}.{img_ext}")
            ...
            if category == "figure":
                new_path = os.path.join(figures_folder, os.path.basename(image_filename))
            ...
            image_data.append([page_num + 1, os.path.basename(new_path), category, caption])
    
    df = pd.DataFrame(image_data, columns=["Page", "Filename", "Category", "Caption"])
    df.to_csv(metadata_file, index=False)
    print(f"Extracted and classified {len(image_data)} images. Metadata saved to {metadata_file}.")
    ...
    for page_num, img in enumerate(images):
        ...
        for i, cnt in enumerate(contours):
            ...
            if w > 100 and h > 50:  # Adjust size threshold for tables
                ...
                table_filename = os.path.join(output_folder, f"page_{page_num+1}_table_{i+1}.png")
        ...
        annotated_filename = os.path.join(annotated_output_folder, f"page_{page_num+1}_annotated.png")
...
def extract_captions_from_pdf(pdf_path, keywords=["Figure", "Fig.", "fig."]):
    ...
    for page_num, img in tqdm(enumerate(images), total=len(images), desc="Processing pages"):
...
def extract_visual_elements_with_gpt4v(pdf_path, output_folder="visual_elements"):
    ...
    for page_num, img in tqdm(enumerate(images), total=len(images), desc="Processing pages"):
        ...
        try:
            ...
            for i, element in enumerate(elements):
                ...
                element_path = os.path.join(elements_folder, f"page_{page_num+1}_{element_type}_{i+1}.png")
                ...
                cv2.putText(img_annotated, element_type, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
        ...
        annotated_path = os.path.join(annotated_folder, f"page_{page_num+1}_annotated.png")
    ...
    print(f"Extracted {len(visual_elements)} visual elements. Metadata saved to {metadata_path}.")
    return visual_elements


def extract_top_portion_of_first_page(pdf_path, output_path=None, top_percentage=0, bottom_percentage=50, compression_quality=100, max_size_bytes=1024*1024):
    """
    ...
    while True:
        ...
        if scale != 1.0:
            new_size = (int(cropped_img.shape[1] * scale), int(cropped_img.shape[0] * scale))
            resized_img = cv2.resize(cropped_img, new_size, interpolation=cv2.INTER_AREA)
        else:
            resized_img = cropped_img
        
        # Encode image
        success, img_bytes = cv2.imencode('.png', resized_img, [cv2.IMWRITE_PNG_COMPRESSION, quality])
        if not success:
        ...
        if scale < 0.1:  # Prevent infinite loop
            raise ValueError("Could not achieve target file size even with maximum compression")
...
def create_pdf_thumbnail(pdf_path, output_path, prct_range=30, max_size_bytes=1024*1024):
    """
    ...
    img_bytes = extract_top_portion_of_first_page(pdf_path, output_path, blank_percentage-1, blank_percentage+prct_range, max_size_bytes=max_size_bytes)
    return img_bytes
...
def compress_pdf(input_path, output_path=None, compression_level=2, remove_images=False):
    """
        ...
        for page_num in range(len(doc)):
            ...
            for img_idx, img in enumerate(image_list):
                ...
                try:
                    ...
                    print(f"Warning: Could not remove image {img_idx} on page {page_num + 1}: {str(e)}")
                    continue
            ...
            for img_idx, img in enumerate(image_list):
                ...
                try:
                    ...
                    if base_image is None:
                        print(f"Warning: Could not extract image {img_idx} on page {page_num + 1}")
                        continue
                    ...
                    if pix.width < 1 or pix.height < 1:
                        print(f"Warning: Invalid image dimensions for image {img_idx} on page {page_num + 1}")
                        continue
                    ...
                    try:
                        ...
                        print(f"Warning: Error processing image {img_idx} on page {page_num + 1}: {str(e)}")
                        continue
                    
                except Exception as e:
                    print(f"Warning: Error extracting image {img_idx} on page {page_num + 1}: {str(e)}")
                    continue
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(PROJECT_DIR, "data", "papers", f"{os.path.basename(paper_path).rsplit('.', 1)[0]}_outputs")
    ...
    img_bytes = create_pdf_thumbnail(paper_path, os.path.join(OUTPUT_DIR, "thumbnail.png"), 30, max_size_bytes=max_size_bytes)
    img = bytes_to_pil_image(img_bytes)
    print(f"Image size: {img.size[0] * img.size[1] * 3 / 1024:.2f} KB")  # Assuming RGB image (3 bytes per pixel)
    img.show()
